  ...
  <div class="form-row">
    <div class="form-group">
      ...
      <input type="text" id="pjt_number" bind:value={data.pjt_number} placeholder="과제 코드를 입력하세요"
        class:invalid={show('pjt_number')} on:blur={() => blur('pjt_number')} />
      {#if show('pjt_number')}<span class="field-error">{errors.pjt_number}</span>{/if}
    </div>
    <div class="form-group">
      ...
      <input type="text" id="pjt_test_number" bind:value={data.pjt_test_number} placeholder="시험 번호를 입력하세요"
        class:invalid={show('pjt_test_number')} on:blur={() => blur('pjt_test_number')} />
      {#if show('pjt_test_number')}<span class="field-error">{errors.pjt_test_number}</span>{/if}
    </div>
  </div>
  <div class="form-group">
    ...
    <input type="text" id="pjt_doc_title" bind:value={data.pjt_doc_title} placeholder="문서 제목을 입력하세요"
      class:invalid={show('pjt_doc_title')} on:blur={() => blur('pjt_doc_title')} />
    {#if show('pjt_doc_title')}<span class="field-error">{errors.pjt_doc_title}</span>{/if}
  </div>
  <div class="form-row">
    <div class="form-group">
      ...
      <input type="text" id="pjt_doc_writer" bind:value={data.pjt_doc_writer} placeholder="연구 담당자를 입력하세요"
        class:invalid={show('pjt_doc_writer')} on:blur={() => blur('pjt_doc_writer')} />
      {#if show('pjt_doc_writer')}<span class="field-error">{errors.pjt_doc_writer}</span>{/if}
    </div>
    <div class="form-group">
      <label for="pjt_doc_count">총 권수</label>
      <input type="number" id="pjt_doc_count" bind:value={data.pjt_doc_count} min="1"
        class:invalid={show('pjt_doc_count')} on:blur={() => blur('pjt_doc_count')} />
      {#if show('pjt_doc_count')}<span class="field-error">{errors.pjt_doc_count}</span>{/if}
    </div>
  </div>
</div>
