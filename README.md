...
- **스트리밍 응답**: PDF를 메모리에서 생성해 직접 전송 — 임시 파일/디렉토리 없음
- **구조화 로깅**: slog JSON lines + 크기 로테이션(lumberjack), 요청별 request ID 추적(`X-Request-ID`), 처리시간 포함 액세스 로그
- **시스템 로그 뷰어**: 인앱 모달에서 **한글 렌더** 로그 조회 — 레벨 뱃지(디버그·정보·경고·오류), request ID 클릭 필터, 원문(raw JSON) 토글, 검색·다운로드·초기화
...
│   └── logging/                 # slog JSON lines → 파일(lumberjack 로테이션) + stdout
├── web/
...
make test     # 백엔드(라벨/PDF/QR/HTTP) + 프론트 계약 패리티
make build    # 프로덕션 빌드
...
**Docker** — 3-stage 빌드(`FROM scratch` 런타임, 약 40MB, KST tzdata 포함). 로그는 `/data` 볼륨에 JSON lines로 영속:
```bash
docker compose up -d --build     # http://localhost:8080
```
scratch 이미지엔 셸이 없으므로 헬스체크는 외부에서 `GET /api/health`로 수행한다.
...
| GET | `/api/logs` | 로그 조회(`lines`/`level`/`request_id` 쿼리) — JSON 파싱된 entry 배열 반환 |
...
| `LOG_FILE` | `logs/app.log` | 로그 파일 경로(인앱 뷰어가 읽음, JSON lines) |
| `LOG_MAX_SIZE_MB` | `10` | 로그 파일 로테이션 크기(MB) |
| `LOG_MAX_BACKUPS` | `5` | 로테이션 백업 보존 개수 |
...
### v3.1.0 (2026-07-20)
**PDF 라벨 출력 + 구조화 로깅**
- 📄 **라벨 출력 xlsx → PDF 전환**: Times·바탕 폰트 임베드, 한/영 run 분리 폰트 라우팅, 텍스트 자동 축소, A4 셸프 패킹(5mm 간격), 실물 인쇄 실측 보정(7cm 라벨 47×150mm), 바탕 합성 볼드
- 📊 **구조화 로깅**: 커스텀 텍스트 로거 → stdlib `slog` JSON lines + lumberjack 크기 로테이션(10MB×5, env 조정). 로그 경로 쓰기 불가 시 기동 실패(fail-fast)
- 🔍 **요청 추적**: 요청별 `X-Request-ID`, 액세스 로그(비즈니스 라우트만 — 로그 뷰어·정적 파일 제외, `duration_ms` 포함)
- 🇰🇷 **로그 뷰어 개편**: 한글 메시지 렌더(프론트 카탈로그), 레벨 뱃지 5종, request ID 클릭 필터, 원문 토글, 클라이언트 검색(한글+raw). 초기화 = 로테이션 백업까지 삭제
- 🧹 레거시 Python 앱·xlsx 패리티 자산 제거, 문서 Go+PDF 기준 갱신
