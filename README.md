...
> 연구소 바인더 라벨 제작용 웹 앱 — 기기·과제 문서 라벨을 표준 포맷으로 만들고, **QR 코드를 임베드한 Excel(.xlsx)** 로 출력한다.
...
**백엔드** Go 1.26 + [Fiber](https://gofiber.io) · Excel [excelize](https://github.com/xuri/excelize) · QR [go-qrcode](https://github.com/skip2/go-qrcode) · 인코딩 `golang.org/x/text`(CP949)
...
│   ├── excel/                   # generator.go, styles.go, geometry.go(중앙 배치), 패리티 테스트
...
> 레거시 Python 앱(`app.py` 등)은 패리티 비교용으로 저장소에 함께 보존되어 있다(은퇴 예정).
...
```bash
go test ./...                          # 백엔드(라벨/엑셀/QR/HTTP) + 골든 패리티 + 계약 패리티
cd web/frontend && npm test            # 프론트 도메인 단위 테스트(vitest)
cd web/frontend && npm run build       # 프론트 타입체크(svelte-check) + 빌드
```

Excel 출력은 레거시 Python 앱의 골든 파일(`testdata/golden/`, 14개)과 의미 단위(셀값·병합·치수·테두리·폰트)로 비교해 패리티를 검증한다. (QR 위치는 v3.0에서 의도적으로 박스 중앙으로 이동 — 전용 중앙 정렬 테스트로 검증.)
